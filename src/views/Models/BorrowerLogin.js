...
const BorrowerLogin = props => {
    ...
    const { errors, touched, isSubmitting, getFieldProps, handleSubmit } = useFormik({
        ...
        onSubmit: (values, { setSubmitting }) => {
            BorrowerService.login(values).then(res => {
                console.log(res)
                if (res.success === 1) {
                    ...
                    <Col sm={6}>
                        <div className="px-4">
                            <Form className="mb-5" onSubmit={handleSubmit}>
                                <FormGroup>
                                    <Input type="email" placeholder="Email" {...getFieldProps('email')} invalid={Boolean(touched.email && errors.email)} />
                                    {touched.email && errors.email && <FormFeedback valid={false}>{errors.email}</FormFeedback>}
                                </FormGroup>
                                <FormGroup className="mb-4">
                                    <Input type="password" placeholder="Password" {...getFieldProps('password')} invalid={Boolean(touched.password && errors.password)} />
                                    {touched.password && errors.password && <FormFeedback valid={false}>{errors.password}</FormFeedback>}
                                </FormGroup>
                                ...
                                <Button disabled={isSubmitting} block>
                                    {
                                        isSubmitting
                                            ...
                                            : (
                                                <React.Fragment>Login <i className="fas fa-sign-in-alt ml-2"></i></React.Fragment>
                                            )
                                    }
                                </Button>
                            </Form>
                            <p className="text-center">New to Wechainbiz ?   <span className="anchor" onClick={() => {
                                toggle();
                                toggleSingUp();
                            }}>Sign up</span> </p>
                        </div>
